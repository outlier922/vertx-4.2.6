...
public class SM3Authentication {
  ...
  /**
   * Encodes user/password/salt information in the following way: SM3(SM3(password
   * + user) + salt)
   *
   * @param user     The connecting user.
   * @param password The connecting user's password.
   * @param salt     A four-salt sent by the server.
   * @return A 35-byte array, comprising the string "sm3" and an SM3 digest.
   */
  public static byte[] encode(String user, String password, byte[] salt) {
    byte[] temp_digest;
    byte[] pass_digest;
    byte[] hex_digest = new byte[67];

    try {
      temp_digest = hash(byteMerge(password.getBytes("UTF-8"), user.getBytes("UTF-8")));
      String temp_digest_hex = byteArrayToHexString(temp_digest);

      pass_digest = hash(byteMerge(temp_digest_hex.getBytes(), salt));
      String pass_digest_hex = byteArrayToHexString(pass_digest);
      System.arraycopy(pass_digest_hex.getBytes(), 0, hex_digest, 3, 64);
  ...
  public static byte[] byteMerge(byte[] by1, byte[] by2) {
    byte[] by3 = new byte[by1.length + by2.length];
    System.arraycopy(by1, 0, by3, 0, by1.length);
    System.arraycopy(by2, 0, by3, by1.length, by2.length);
    return by3;
  }
